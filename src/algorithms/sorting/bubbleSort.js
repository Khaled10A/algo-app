...
export function bubbleSortDebug(arr) {
  ...
  const snap = (activeLine, vars, log) => steps.push({
    ...
    callStack: ["bubbleSort(arr)", vars.j !== undefined ? "  └ inner for(j=0..n-i-1)" : "  └ outer for(i=0..n-1)"],
    log,
  });
  ...
  for (let i = 0; i < a.length - 1; i++) {
    ...
    for (let j = 0; j < a.length - i - 1; j++) {
      snap(3, { i, j, swapped: false }, `Compare A[${j}]=${a[j]} with A[${j+1}]=${a[j+1]}`);
      if (a[j] > a[j + 1]) {
        [a[j], a[j + 1]] = [a[j + 1], a[j]];
        snap(4, { i, j, swapped: true }, `Swap! A[${j}]=${a[j]} ↔ A[${j+1}]=${a[j+1]}`);
      }
    }
  }
