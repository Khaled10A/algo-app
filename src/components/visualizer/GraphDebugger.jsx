...
const MODES = [
  {
    id: "select",
    label: "Select",
    hint: "Click a node to set the source, click an edge to edit it",
  },
  {
    id: "add-node",
    label: "Place node",
    hint: "Click the canvas to place a new node",
  },
  { id: "add-edge", label: "Connect", hint: "Click two nodes to connect them" },
  {
    id: "delete",
    label: "Delete",
    hint: "Click a node or an edge to remove it",
  },
];
...
function freeSpot(positions) {
  ...
  for (let y = 60; y <= 260; y += 66) {
    for (let x = 60; x <= 340; x += 70) {
      if (
        !taken.some((pt) => Math.abs(pt.x - x) < 45 && Math.abs(pt.y - y) < 45)
      ) {
        return { x, y };
      }
    }
...
export function GraphDebugger({ isDark }) {
  ...
  const [positions, setPositions] = useState(() =>
    positionsFor(createDefaultGraph()),
  );
  ...
  function handleAddEdge() {
    ...
    applyMutation(
      (g) => addEdge(g, edgeFrom, edgeTo, weight),
      `Edge ${edgeFrom} — ${edgeTo} added`,
    );
  }
  ...
  function handleUpdateWeight() {
    ...
    applyMutation(
      (g) => setEdgeWeight(g, edgeFrom, edgeTo, weight),
      `Edge ${edgeFrom} — ${edgeTo} weight set to ${weight}`,
    );
  }
  ...
  function handleRemoveEdge() {
    ...
    applyMutation(
      (g) => removeEdge(g, edgeFrom, edgeTo),
      `Edge ${edgeFrom} — ${edgeTo} removed`,
    );
  }
    ...
    <div>
          ...
          <div style={{ display: "flex", gap: 5 }}>
            {[
              ...
              <button
                ...
                aria-pressed={algoId === id}
                style={btnStyle(algoId === id)}
              >
                {GRAPH_DESCRIPTORS[id].name}
              </button>
            ))}
          </div>
        </div>

        {algoId !== "floyd-warshall" && algoId !== "kruskal" && (
          <div>
            <div
              style={{
                fontSize: 11,
            ...
            >
              {nodeIds.map((n) => (
                <button
                  ...
                  onClick={() => {
                    setStartNode(n);
                    setNoticeMsg(`Source node: ${n}`, "ok");
                  }}
                  aria-pressed={startNode === n}
                  style={btnStyle(startNode === n)}
                >
                  {n}
                </button>
              ))}
      ...
      {algoId === "prim" &&
        ...
        current.treeCount > 1 && (
          ...
          >
            Graph is disconnected — growing tree {current.treeCount}. The result
      ...
      >
          ...
          <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
            <select
              ...
              onChange={(e) => {
                ...
                setSelectedEdge([e.target.value, edgeTo]);
              }}
              style={selectStyle}
            >
              {nodeIds.map((n) => (
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setSelectedEdge([edgeFrom, e.target.value]);
              }}
              style={selectStyle}
            >
              {nodeIds.map((n) => (
            ...
            />
            <button onClick={handleAddEdge} style={smallBtn}>
              Add edge
            </button>
            <button onClick={handleUpdateWeight} style={smallBtn}>
              Update weight
            </button>
            <button
              onClick={handleRemoveEdge}
              style={{ ...smallBtn, color: p.red }}
            >
              Remove edge
            </button>
          </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 5 }}>
            <button onClick={handleAddNode} style={smallBtn}>
              Add node
            </button>
            <button onClick={handleReset} style={{ ...smallBtn, color: p.red }}>
              Clear
            </button>
            <button onClick={handleRestoreDefault} style={smallBtn}>
              Restore default
            </button>
          </div>
        </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns:
            steps.length > 0 ? "1fr 1fr" : "minmax(0, 460px)",
          gap: 12,
          alignItems: "start",
        }}
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          >
            ...
            >
              Graph
              {nodeIds.length > 0 && (
                <span style={{ fontWeight: 400, color: textMute }}>
                  {" "}
                  — {nodeIds.length} nodes,{" "}
                  {Math.round(Object.values(graph).flat().length / 2)} edges
                </span>
              )}
            </div>
            <GraphSVG
          ...
          {steps.length > 0 && algoId !== "floyd-warshall" && (
            <div
              className="glass-floating"
              style={{ borderRadius: 12, padding: "13px 14px" }}
            >
              <div
                style={{
                  fontSize: 11,
                  ...
                  >
                    <div
                      style={{
                        ...
                        borderRadius: "50%",
                        background:
                          n === current?.current ? accentColor : p.green,
                        display: "flex",
                        alignItems: "center",
                    ...
                    {i < (current?.visitOrder?.length || 0) - 1 && (
                      <span style={{ color: textMute, fontSize: 12 }}>→</span>
                    )}
                  </div>
                ))}
                {!current?.visitOrder?.length && (
                  <span style={{ color: textMute, fontSize: 11 }}>
                    Not started yet...
                  </span>
                )}
              </div>
        ...
        {steps.length === 0 ? (
          <div
            className="surface-card panel-in"
            style={{
              borderRadius: 12,
              display: "flex",
          ...
          >
            ...
            <div style={{ fontSize: 13, color: textMute }}>
              Select an algorithm and click Generate
            </div>
            <div style={{ fontSize: 11, color: textMute }}>
              …or build a graph with the editor and watch it come alive.
            </div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            >
              ...
              >
                ...
                >
                  {descriptor.name.replace(/ /g, "_").toLowerCase()}.js
                </span>
                <span
                  style={{
                    fontSize: 9,
                  ...
                  >
                    <span
                      style={{
                        ...
                        fontSize: 9,
                        color:
                          n === current?.activeLine ? accentColor : textMute,
                        fontFamily: "monospace",
                        flexShrink: 0,
                        userSelect: "none",
                      }}
                  ...
                  {algoId === "dfs" ? (
                    ...
                    >
                      {(current?.callStack || []).length === 0 ? (
                        <span style={{ fontSize: 10, color: textMute }}>
                          Empty
                        </span>
                      ) : (
                        (current?.callStack || []).map((n, i) => (
                          <div
                            ...
                            style={{
                              background:
                                i === (current?.callStack?.length || 0) - 1
                                  ? `${accentColor}20`
                                  : codeBg,
              ...
              >
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  (current.mstEdges || []).length === 0 ? (
                    ...
                    [
                      ...(current.mstEdges || []).map((e) => ({
                        ...e,
                        state: "mst",
                      })),
                      ...(current.frontier || []).map((e) => ({
                        ...e,
                        state: "pending",
                      })),
                      ...(current.rejectedEdge
                        ? [
                            {
                              from: current.rejectedEdge[0],
                              to: current.rejectedEdge[1],
                              state: "rejected",
                            },
                          ]
                        : []),
                      ...
                      .sort(
                        (a, b) =>
                          a.weight - b.weight || (a.from < b.from ? -1 : 1),
                      )
                      .map((e, i) => (
                        <div
                          key={i}
                            ...
                            color:
                              e.state === "mst"
                                ...
                                : e.state === "rejected"
                                  ...
                                  : textMute,
                            textDecoration:
                              e.state === "rejected" ? "line-through" : "none",
                          }}
                        >
                          {e.from}—{e.to} · {e.weight}
                          {e.state === "mst"
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {Object.entries(current.distances).map(([node, d]) => (
                      ...
                      >
                        ...
                        <span
                          style={{
                            color: !Number.isFinite(d) ? p.red : accentColor,
                            fontWeight: Number.isFinite(d) ? 700 : 400,
                          }}
                        >
...
function GraphSVG({
  ...
  const edgeStyle = (e) => {
    ...
    const isSelected =
      selectedEdge && edgeKey(selectedEdge[0], selectedEdge[1]) === key;
    const isCurrent =
      currentEdge && edgeKey(currentEdge[0], currentEdge[1]) === key;
    const isRelaxed =
      relaxedEdge && edgeKey(relaxedEdge[0], relaxedEdge[1]) === key;
    ...
    if (mstEdges) {
      const key = edgeKey(e.from, e.to);
      const isCandidate =
        candidateEdge && edgeKey(candidateEdge[0], candidateEdge[1]) === key;
      const isRejected =
        rejectedEdge && edgeKey(rejectedEdge[0], rejectedEdge[1]) === key;
  ...
  return (
    <svg
      ...
      style={{
        ...
        cursor: mode === "add-node" ? "copy" : "default",
      }}
      role="img"
      aria-label="Graph playground"
      onClick={(evt) => {
    ...
    >
      ...
      {edges.map((e) => {
        ...
        const clickable =
          Boolean(onEdgeClick) && (mode === "select" || mode === "delete");
        return (
          <g key={e.key}>
            <line
              x1={e.a.x}
      ...
      {nodeIds.map((node) => {
        ...
        const distLabel =
          dist === undefined || !Number.isFinite(dist) ? "∞" : String(dist);
