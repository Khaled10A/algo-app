...
export default function App() {
  ...
  const subTab = domain.subTabs.includes(subTabRaw)
    ? subTabRaw
    : domain.subTabs[0];
  const showInspector =
  ...
  const [selSort, setSelSort] = usePersistentState("sort:algos", [
    "insertion-sort",
    "merge-sort",
    "quick-sort",
  ]);
  const [sortTypes, setSortTypes] = usePersistentState("sort:types", [
    "random",
    "sorted",
    "reverse",
    "nearly",
  ]);
  const [sortSizes, setSortSizes] = usePersistentState(
    "sort:sizes",
    "50,100,150",
  );
  const [sortMetric, setSortMetric] = usePersistentState("sort:metric", "time");
  const [sortInputMode, setSortInputMode] = usePersistentState(
    "sort:input-mode",
    "random",
  );
  const [customArrayStr, setCustomArrayStr] = usePersistentState(
    "sort:custom-array",
    "34,7,23,32,5,62,32,14,7,89,21,45",
  );

  // STRING MATCHING BENCHMARK SETTINGS
  const [selSearch, setSelSearch] = usePersistentState("search:algos", [
    "brute-force",
    "horspool",
    "kmp",
  ]);
  const [searchScs, setSearchScs] = usePersistentState("search:scenarios", [
    "start",
    "end",
    "multiple",
    "nomatch",
  ]);
  const [searchSizes, setSearchSizes] = usePersistentState(
    "search:sizes",
    "300,500",
  );
  const [pattern, setPattern] = usePersistentState("search:pattern", "algo");
  const [searchInputMode, setSearchInputMode] = usePersistentState(
    "search:input-mode",
    "generate",
  );
  const [searchMetric, setSearchMetric] = usePersistentState(
    "search:metric",
    "time",
  );
  ...
  const [storedVizSpeed, setStoredVizSpeed] = usePersistentState(
    "viz:speed",
    80,
  );
  const [vizSteps, setVizSteps] = useState([]);
  const vizPlayback = usePlayback({
    length: vizSteps.length,
    initialSpeed: storedVizSpeed,
  });
  ...
  // PSEUDOCODE
  const [pseudoAlgo, setPseudoAlgo] = usePersistentState(
    "pseudo:algo",
    "insertion-sort",
  );
  ...
  const sortLineRef = useRef();
  const sortBarRef = useRef();
  const srchLineRef = useRef();
  const srchBarRef = useRef();
        ...
        >
          ...
          <main
            style={{
              flex: 1,
              overflowY: "auto",
              padding: showInspector
                ? "10px 34px 44px 328px"
                : "14px 40px 44px",
              minWidth: 0,
              height: "100%",
            }}
            ...
            <div key={domain.id + ":" + subTab} className="panel-in">
              ...
              {subTab === "benchmark" &&
                ...
                (searchResults ? (
                  ...
                  <Empty
                    icon="🔍"
                    text="Configure & Run String Matching Benchmark"
                  />
                ))}
              {subTab === "visualizer" && (
                <VisualizerTab
            ...
            >
              ...
              >
                <a
                  ...
                  onMouseLeave={(e) =>
                    (e.currentTarget.style.color = p.textFaint)
                  }
                >
                  © Khaled Alnajjar
                </a>
