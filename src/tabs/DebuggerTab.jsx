...
const ARRAY_SEARCH_ALGOS = getWithDebug("searching").filter(
  (d) => d.group === "array",
);
const STRING_MATCH_ALGOS = getWithDebug("searching").filter(
  (d) => d.group === "string",
);
...
export function DebuggerTab({ isDark }) {
  ...
  const isStringSearch =
    descriptor.category === "searching" && descriptor.group === "string";
  ...
  function generate() {
    ...
    if (isStringSearch) {
      ...
      if (isBinary) {
        ...
        const target =
          binaryTargetMode === "missing" ? sortedMax + 1 : undefined;
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
              {[
                ...
                <button
                  key={v}
                  onClick={() => setArrInputMode(v)}
                  aria-pressed={arrInputMode === v}
                  style={{
                    padding: "3px 10px",
                    borderRadius: 5,
                    border: `1px solid ${arrInputMode === v ? accentColor : p.border}`,
                    background:
                      arrInputMode === v ? `${accentColor}20` : "transparent",
            ...
            {arrInputMode === "auto" ? (
              ...
              <div>
                ...
                <input
                  value={customArrStr}
                  onChange={(e) => setCustomArrStr(e.target.value)}
                ...
                />
                <div
                  style={{ fontSize: 9, color: p.textSecondary, marginTop: 3 }}
                >
                  {
                    customArrStr
                      .split(",")
            ...
            {isBinary && (
              <div style={{ marginTop: 8 }}>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {[
                    ["present", "In array"],
                    ["missing", "Not in array"],
                  ].map(([v, l]) => (
                    <button
                      key={v}
                      onClick={() => setBinaryTargetMode(v)}
                      aria-pressed={binaryTargetMode === v}
                      style={{
                        padding: "3px 10px",
                        borderRadius: 5,
            ...
            <div>
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      padding: "3px 10px",
                      borderRadius: 5,
                      border: `1px solid ${debugTextMode === v ? accentColor : p.border}`,
                      background:
                        debugTextMode === v
                          ? `${accentColor}20`
                          : "transparent",
                      color:
                        debugTextMode === v ? accentColor : p.textSecondary,
                      fontSize: 9,
                      cursor: "pointer",
                      fontFamily: "monospace",
                    }}
              ...
              {debugTextMode === "file" ? (
                <label style={{ cursor: "pointer" }}>
                  <div
                    style={{
                      border: `2px dashed ${debugFileName ? p.green : p.borderStrong}`,
                      borderRadius: 7,
                      padding: "8px 12px",
                      textAlign: "center",
                      background: debugFileName ? p.accentTint : "transparent",
                      minWidth: 180,
                      transition: "all 0.2s",
                    }}
                  >
                    ...
                    <div
                      style={{
                        fontSize: 9,
                        color: debugFileName ? p.green : p.textSecondary,
                        fontFamily: "monospace",
                        marginTop: 2,
                      }}
                    >
                    ...
                    onChange={(e) => {
                      ...
                      reader.onload = (ev) => {
                        ...
                        if (content.length > MAX_DEBUG_TEXT) {
                          ...
                          setInputNotice(
                            `File truncated to the first ${MAX_DEBUG_TEXT.toLocaleString()} characters for debugging.`,
                          );
                        } else {
