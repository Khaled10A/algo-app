...
const QUICK_ACTIONS = {
  ar: [
    {
      label: "⚡ شرح KMP",
      prompt: "اشرح كيف يعمل KMP ولماذا هو أسرع من Brute Force.",
    },
    {
      label: "📊 مقارنة الترتيب",
      prompt: "قارن بين Merge Sort و Quick Sort — متى أستخدم كل منهما؟",
    },
    {
      label: "🎯 الأفضل للبيانات العشوائية؟",
      prompt: "أي خوارزمية ترتيب هي الأفضل للبيانات العشوائية ولماذا؟",
    },
    {
      label: "🔍 Horspool مقابل KMP",
      prompt: "ما الفرق بين Horspool و KMP في مطابقة النصوص؟",
    },
    {
      label: "📈 ملخص Big-O",
      prompt: "أعطني ملخصاً سريعاً لتعقيد Big-O لجميع خوارزميات الترتيب.",
    },
    {
      label: "🧩 شرح جدول LPS",
      prompt: "اشرح ما هو جدول LPS في KMP وكيف يساعد.",
    },
  ],
  en: [
    {
      label: "⚡ Explain KMP",
      prompt:
        "Explain how KMP search works and why it's faster than brute force.",
    },
    {
      label: "📊 Compare sorts",
      prompt: "Compare Merge Sort vs Quick Sort — when should I use each?",
    },
    {
      label: "🎯 Best for random data?",
      prompt:
        "Which sorting algorithm is best for random unsorted data and why?",
    },
    {
      label: "🔍 Horspool vs KMP",
      prompt:
        "What is the difference between Horspool and KMP string matching?",
    },
    {
      label: "📈 Big-O summary",
      prompt:
        "Give me a quick Big-O complexity summary for all sorting algorithms.",
    },
    {
      label: "🧩 Explain LPS table",
      prompt: "Explain what the LPS table is in KMP and how it helps.",
    },
  ],
  fr: [
    {
      label: "⚡ Expliquer KMP",
      prompt:
        "Explique comment fonctionne KMP et pourquoi c'est plus rapide que Brute Force.",
    },
    {
      label: "📊 Comparer les tris",
      prompt: "Compare Merge Sort et Quick Sort — quand utiliser chacun?",
    },
    {
      label: "🎯 Meilleur pour données aléatoires?",
      prompt:
        "Quel algorithme de tri est le meilleur pour des données aléatoires?",
    },
    {
      label: "🔍 Horspool vs KMP",
      prompt: "Quelle est la différence entre Horspool et KMP?",
    },
    {
      label: "📈 Résumé Big-O",
      prompt:
        "Donne-moi un résumé rapide de la complexité Big-O de tous les algorithmes.",
    },
    {
      label: "🧩 Table LPS",
      prompt: "Explique ce qu'est la table LPS dans KMP et comment elle aide.",
    },
  ],
  de: [
    {
      label: "⚡ KMP erklären",
      prompt:
        "Erkläre wie KMP funktioniert und warum es schneller als Brute Force ist.",
    },
    {
      label: "📊 Sortierungen vergleichen",
      prompt:
        "Vergleiche Merge Sort und Quick Sort — wann verwende ich welchen?",
    },
    {
      label: "🎯 Bester für zufällige Daten?",
      prompt: "Welcher Sortieralgorithmus ist am besten für zufällige Daten?",
    },
    {
      label: "🔍 Horspool vs KMP",
      prompt: "Was ist der Unterschied zwischen Horspool und KMP?",
    },
    {
      label: "📈 Big-O Zusammenfassung",
      prompt: "Gib mir eine schnelle Big-O Komplexitätszusammenfassung.",
    },
    {
      label: "🧩 LPS Tabelle",
      prompt: "Erkläre was die LPS-Tabelle in KMP ist und wie sie hilft.",
    },
  ],
};
...
function RecommendForm({
  ...
  const LABELS = {
    ar: {
      title: "🎯 موصي الخوارزميات",
      task: "المهمة",
      size: "حجم البيانات",
      order: "ترتيب البيانات",
      priority: "الأولوية",
      btn: "⚡ احصل على التوصية",
      sorting: "ترتيب",
      matching: "مطابقة نصوص",
      small: "صغير",
      medium: "متوسط",
      large: "كبير",
      random: "عشوائي",
      nearly_sorted: "شبه مرتب",
      sorted: "مرتب",
      reverse: "معكوس",
      speed: "السرعة",
      memory: "ذاكرة منخفضة",
    ...
    fr: {
      title: "🎯 RECOMMANDEUR D'ALGORITHMES",
      task: "TÂCHE",
      size: "TAILLE DES DONNÉES",
      order: "ORDRE DES DONNÉES",
      priority: "PRIORITÉ",
      btn: "⚡ OBTENIR UNE RECOMMANDATION",
      sorting: "Tri",
      matching: "Recherche de chaînes",
      small: "Petit",
      medium: "Moyen",
      large: "Grand",
      random: "Aléatoire",
      ...
      reverse: "Inversé",
      speed: "Vitesse",
      memory: "Mémoire faible",
      stability: "Stabilité",
    },
    de: {
      title: "🎯 ALGORITHMUS-EMPFEHLER",
      task: "AUFGABE",
      size: "DATENGRÖSSE",
      order: "DATENREIHENFOLGE",
      priority: "PRIORITÄT",
      btn: "⚡ EMPFEHLUNG ERHALTEN",
      sorting: "Sortierung",
      matching: "String-Matching",
      small: "Klein",
      medium: "Mittel",
      large: "Groß",
      random: "Zufällig",
      nearly_sorted: "Fast sortiert",
      sorted: "Sortiert",
      reverse: "Umgekehrt",
      speed: "Geschwindigkeit",
      memory: "Wenig Speicher",
        ...
        {opts.map(([v, l]) => (
          <button
            ...
            style={{
              ...
              border: `1px solid ${val === v ? accentColor : border}`,
  ...
  return (
    ...
    >
      ...
      {sel(L.task, task, setTask, [
        ["sorting", L.sorting],
        ["matching", L.matching],
      ])}
      {sel(L.size, size, setSize, [
        ["small", L.small],
        ["medium", L.medium],
        ["large", L.large],
      ])}
      {sel(L.order, order, setOrder, [
        ["random", L.random],
        ["nearly_sorted", L.nearly_sorted],
        ["sorted", L.sorted],
        ["reverse", L.reverse],
      ])}
      {sel(L.priority, goal, setGoal, [
        ["speed", L.speed],
        ["memory", L.memory],
...
export function AIAssistantTab({ isDark, sortResults, searchResults }) {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: WELCOME_MESSAGES["ar"] },
  ]);
  ...
  function analyzeResults() {
    if (!sortResults && !searchResults) {
      send(
        lang === "ar"
          ? "لم أشغّل أي بنشمارك بعد. ماذا أشغّل أولاً؟"
          : "I haven't run any benchmarks yet. What should I run first?",
      );
      return;
    }
    const parts = [];
    if (sortResults) {
      ...
      algos.forEach((algo) => {
        types.forEach((type) => {
          const vals = results[algo][type]
            .map((r) => `n=${r.n}: ${r.time}ms (${r.comparisons} comparisons)`)
            .join(", ");
    ...
    const prefix =
      lang === "ar"
        ? "حلّل هذه النتائج وأخبرني أي خوارزمية فازت ولماذا:\n\n"
        : lang === "fr"
          ? "Analysez ces résultats et dites-moi quel algorithme a gagné:\n\n"
          : lang === "de"
            ? "Analysiere diese Ergebnisse und sag mir welcher Algorithmus gewonnen hat:\n\n"
        ...
        >
          {lines.map((line, li) => {
            const parts = line.split(/(\*\*[^*]+\*\*)/g).map((p, pi) =>
              p.startsWith("**") ? (
                <strong
                  key={pi}
                  style={{
                    color: isUser ? p2OnAccent(accentColor) : accentColor,
                  }}
                >
                  {p.slice(2, -2)}
                </strong>
            ...
            return (
              <div
                key={li}
                style={{ marginBottom: li < lines.length - 1 ? 4 : 0 }}
              >
                {parts}
              </div>
            );
        ...
        {isUser && (
          <div
            style={{
              ...
              borderRadius: "50%",
              flexShrink: 0,
              background: pf.trackBg,
              border: `1px solid ${pf.borderStrong}`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
            }}
          >
            ...
            >
              <circle
                ...
                cy="8"
                r="3.6"
                stroke={pf.textSecondary}
                strokeWidth="1.8"
              />
              <path
                d="M4.5 20c1.2-3.6 4-5.4 7.5-5.4S18.3 16.4 19.5 20"
                stroke={pf.textSecondary}
                strokeWidth="1.8"
                strokeLinecap="round"
              />
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              background:
                sortResults || searchResults ? pf.accentTint : "transparent",
          ...
          <button
            onClick={() =>
              setMessages([
                { role: "assistant", content: WELCOME_MESSAGES[lang] },
              ])
            }
            style={{
              padding: "5px 12px",
      ...
      >
        {currentActions.map(({ label, prompt }) => (
          <button
            ...
            onMouseEnter={(e) => {
              e.target.style.borderColor = pf.accent;
              e.target.style.color = pf.accentText;
            }}
            onMouseLeave={(e) => {
              e.target.style.borderColor = border;
              e.target.style.color = textMute;
            }}
          >
            {label}
          </button>
        ...
        <button
          ...
          style={{
            ...
            boxShadow:
              loading || !input.trim()
                ? "none"
                : `inset 0 1px 0 rgba(255,255,255,0.25), 0 3px 10px ${pf.accent}44`,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            color: pf.onAccent,
