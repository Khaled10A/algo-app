...
export function ReportTab({ sortResults, searchResults, sortMetric, searchMetric, pattern }) {
  ...
  const secTitle = (t) => (
    <div style={{ fontSize: 11, fontWeight: 600, color: textMuted, marginBottom: 10 }}>{t}</div>
  );
  ...
  if (sortResults) {
    ...
    const resultCard = card(<>
      ...
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8, marginBottom: 14 }}>
        The benchmark evaluated <strong style={{ color: pf.accent }}>{algos.length} sorting algorithms</strong> across{" "}
        <strong style={{ color: pf.textStrong }}>{types.length} input types</strong> and{" "}
        <strong style={{ color: pf.textStrong }}>{sizes.length} input sizes</strong> (n = {sizes.join(", ")}).
        All algorithms were implemented from scratch without library functions.
      </p>
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8, marginBottom: 14 }}>
        <strong style={{ color: "#1f9d48" }}>Best overall: {getAlgorithmForDisplay(best.algo).name}</strong> — average{" "}
        <strong style={{ color: "#1f9d48" }}>{best.avg.toFixed(4)} {unit}</strong>, consistent with its {getAlgorithmForDisplay(best.algo).complexity.average} complexity.
      </p>
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8, marginBottom: 14 }}>
        <strong style={{ color: "#d70015" }}>Worst overall: {getAlgorithmForDisplay(worst.algo).name}</strong> — average{" "}
        <strong style={{ color: "#d70015" }}>{worst.avg.toFixed(4)} {unit}</strong>,{" "}
        approximately <strong style={{ color: "#c93400" }}>{ratio}×</strong> slower than {getAlgorithmForDisplay(best.algo).name}.
      </p>
      {algoStats.length > 2 && (
        <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8, marginBottom: 14 }}>
    ...
    const conclusionCard = card(<>
      ...
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8 }}>
        <strong style={{ color: "#1f9d48" }}>{getAlgorithmForDisplay(best.algo).name}</strong> is recommended for general-purpose sorting due to its consistent {getAlgorithmForDisplay(best.algo).complexity.average} average-case performance.{" "}
      ...
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8, marginTop: 10 }}>
        <strong style={{ color: "#d70015" }}>{getAlgorithmForDisplay(worst.algo).name}</strong> should be avoided for large datasets given its {getAlgorithmForDisplay(worst.algo).complexity.worst} worst-case complexity,
  ...
  if (searchResults) {
    ...
    const resultCard = card(<>
      ...
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8, marginBottom: 14 }}>
        String matching algorithms were benchmarked across <strong style={{ color: "#e8386d" }}>{scenarios.length} scenarios</strong> using text sizes of{" "}
        <strong style={{ color: pf.textStrong }}>{sizes.join(", ")} characters</strong>. Pattern used: <strong style={{ color: "#fbbf24" }}>"{pattern}"</strong>.
      </p>
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8, marginBottom: 14 }}>
        <strong style={{ color: "#1f9d48" }}>Best: {getAlgorithmForDisplay(best.algo).name}</strong> — averaged{" "}
        <strong style={{ color: "#1f9d48" }}>{best.avg.toFixed(4)} {unit}</strong> across all scenarios.
      </p>
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8, marginBottom: 14 }}>
        <strong style={{ color: "#d70015" }}>Most expensive: {getAlgorithmForDisplay(worst.algo).name}</strong> — averaged{" "}
        <strong style={{ color: "#d70015" }}>{worst.avg.toFixed(4)} {unit}</strong>,{" "}
        about <strong style={{ color: "#c93400" }}>{ratio}×</strong> more than {getAlgorithmForDisplay(best.algo).name}.
      </p>
      <div style={{ marginBottom: 12 }}>
    ...
    const conclusionCard = card(<>
      ...
      <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8 }}>
        <strong style={{ color: "#1f9d48" }}>{best.algo}</strong> demonstrated superior performance, especially on longer texts where its preprocessing advantage compounds.
...
function ScalingAssessment({ sortResults, unit, isDark }) {
  ...
  return (
    <p style={{ fontSize: 12, color: isDark ? "#e2e8f0" : "#1e293b", lineHeight: 1.8, marginTop: 10 }}>
      ...
      {" "}<strong style={{ color: "#1f9d48" }}>{d.name}</strong>&apos;s median {unit} grew
      approximately <strong style={{ color: "#c93400" }}>{observed.toFixed(1)}×</strong> — {verdict}.
