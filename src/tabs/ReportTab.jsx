...
export function ReportTab({
  ...
  const textPrimary = pf.textPrimary;
  ...
  if (!sortResults && !searchResults) {
    return (
      ...
      >
        ...
        <div style={{ fontSize: 13, color: pf.textSecondary }}>
  ...
  const card = (children, accent = pf.accent) => (
    <div
      style={{
        background: bg,
  ...
  if (sortResults) {
    ...
    const algoStats = algos
      .map((algo) => {
        const allVals = types.flatMap((t) =>
          sizes.map((_, si) => results[algo][t][si][mk]),
        );
    ...
    const typeWinners = types.map((type) => {
      const winner = algos.reduce((best, algo) => {
        const avg =
          sizes.reduce((s, _, si) => s + results[algo][type][si][mk], 0) /
          sizes.length;
        const bestAvg =
          sizes.reduce((s, _, si) => s + results[best][type][si][mk], 0) /
          sizes.length;
    ...
    const resultCard = card(
      <>
        {secTitle("Sorting — results analysis")}
        <p
          style={{
            fontSize: 12,
            color: textPrimary,
        ...
        >
          ...
          <strong style={{ color: pf.accent }}>
            {algos.length} sorting algorithms
          </strong>{" "}
          across{" "}
          <strong style={{ color: pf.textStrong }}>
            {types.length} input types
          </strong>{" "}
          and{" "}
          <strong style={{ color: pf.textStrong }}>
            {sizes.length} input sizes
          </strong>{" "}
          (n = {sizes.join(", ")}). All algorithms were implemented from scratch
        ...
        >
          <strong style={{ color: "#1f9d48" }}>
            Best overall: {getAlgorithmForDisplay(best.algo).name}
          </strong>{" "}
          — average{" "}
          <strong style={{ color: "#1f9d48" }}>
            {best.avg.toFixed(4)} {unit}
          </strong>
          , consistent with its{" "}
          {getAlgorithmForDisplay(best.algo).complexity.average} complexity.
        </p>
        <p
          style={{
            fontSize: 12,
        ...
        >
          <strong style={{ color: "#d70015" }}>
            Worst overall: {getAlgorithmForDisplay(worst.algo).name}
          </strong>{" "}
          — average{" "}
          <strong style={{ color: "#d70015" }}>
            {worst.avg.toFixed(4)} {unit}
          </strong>
          , approximately <strong style={{ color: "#c93400" }}>{ratio}×</strong>{" "}
          slower than {getAlgorithmForDisplay(best.algo).name}.
        </p>
        {algoStats.length > 2 && (
          <p
            style={{
          ...
          >
            ...
            {algoStats
              .slice(1, -1)
              .map((s) => `${s.avg.toFixed(4)} ${unit}`)
              .join(" and ")}{" "}
            respectively.
          </p>
        )}
        <div style={{ marginBottom: 12 }}>
          {secTitle("Input type winners")}
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {typeWinners.map(({ type, winner }) => (
              <div
                key={type}
                style={{
                  background: pf.surfaceAlt,
                  border: `1px solid ${border}`,
                  borderRadius: 7,
                  padding: "7px 12px",
                }}
              >
                ...
                <div
                  style={{ fontSize: 11, color: "#1f9d48", fontWeight: "bold" }}
                >
                  {getAlgorithmForDisplay(winner).name}
                </div>
              </div>
            ))}
          </div>
    ...
    const conclusionCard = card(
      <>
        {secTitle("Sorting — conclusion")}
        <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8 }}>
          <strong style={{ color: "#1f9d48" }}>
            {getAlgorithmForDisplay(best.algo).name}
          </strong>{" "}
          is recommended for general-purpose sorting due to its consistent{" "}
          {getAlgorithmForDisplay(best.algo).complexity.average} average-case
          performance.{" "}
          {getAlgorithmForDisplay(best.algo).complexity.paradigm ===
          "Divide & Conquer"
        ...
        >
          <strong style={{ color: "#d70015" }}>
            {getAlgorithmForDisplay(worst.algo).name}
          </strong>{" "}
          should be avoided for large datasets given its{" "}
          {getAlgorithmForDisplay(worst.algo).complexity.worst} worst-case
          complexity, though it remains acceptable for very small inputs (n ≤
          20).
        </p>
        <ScalingAssessment
  ...
  if (searchResults) {
    ...
    const algoStats = algos
      .map((algo) => {
        const allVals = scenarios.flatMap((sc) =>
          sizes.map((_, si) => results[algo][sc][si][mk]),
        );
        const avg = allVals.reduce((a, b) => a + b, 0) / allVals.length;
        return { algo, avg };
      })
    ...
    const scWinners = scenarios.map((sc) => {
      const winner = algos.reduce((best, algo) => {
        const avg =
          sizes.reduce((s, _, si) => s + results[algo][sc][si][mk], 0) /
          sizes.length;
        const bestAvg =
          sizes.reduce((s, _, si) => s + results[best][sc][si][mk], 0) /
          sizes.length;
        return avg < bestAvg ? algo : best;
      }, algos[0]);
      return { sc, winner };
    ...
    const resultCard = card(
      <>
        {secTitle("String matching — results analysis")}
        <p
          style={{
            fontSize: 12,
            color: textPrimary,
        ...
        >
          String matching algorithms were benchmarked across{" "}
          <strong style={{ color: "#e8386d" }}>
            {scenarios.length} scenarios
          </strong>{" "}
          using text sizes of{" "}
          <strong style={{ color: pf.textStrong }}>
            {sizes.join(", ")} characters
          </strong>
          . Pattern used:{" "}
        ...
        >
          <strong style={{ color: "#1f9d48" }}>
            Best: {getAlgorithmForDisplay(best.algo).name}
          </strong>{" "}
          — averaged{" "}
          <strong style={{ color: "#1f9d48" }}>
            {best.avg.toFixed(4)} {unit}
        ...
        >
          <strong style={{ color: "#d70015" }}>
            Most expensive: {getAlgorithmForDisplay(worst.algo).name}
          </strong>{" "}
          — averaged{" "}
          <strong style={{ color: "#d70015" }}>
            {worst.avg.toFixed(4)} {unit}
          </strong>
          , about <strong style={{ color: "#c93400" }}>{ratio}×</strong> more
          than {getAlgorithmForDisplay(best.algo).name}.
        </p>
        <div style={{ marginBottom: 12 }}>
          {secTitle("Scenario winners")}
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {scWinners.map(({ sc, winner }) => (
              ...
              >
                ...
                <div
                  style={{ fontSize: 11, color: "#e8386d", fontWeight: "bold" }}
                >
                  {winner}
                </div>
              </div>
    ...
    const conclusionCard = card(
      <>
        {secTitle("String matching — conclusion")}
        <p style={{ fontSize: 12, color: textPrimary, lineHeight: 1.8 }}>
          <strong style={{ color: "#1f9d48" }}>{best.algo}</strong> demonstrated
          superior performance, especially on longer texts where its
          preprocessing advantage compounds. Algorithms with preprocessing (KMP,
          Horspool) are recommended for production use.
        </p>
        <p
          style={{
            fontSize: 12,
        ...
        >
          Brute Force remains competitive on short patterns and small texts,
          with O(1) space complexity making it suitable for memory-constrained
          environments.
        </p>
        <p
          style={{
        ...
        >
          In the "No Match" scenario, preprocessing-based algorithms show the
          most improvement over Brute Force, as their skip mechanisms avoid
          exhaustive scanning.
        </p>
      </>,
      "#f472b6",
  ...
  return (
    <div>
      ...
      <div
        style={{
          background: pf.accentTint,
          border: `1px solid ${pf.accent}40`,
          borderRadius: 8,
          padding: "10px 16px",
          marginBottom: 20,
          fontSize: 12,
...
function ScalingAssessment({ sortResults, unit, isDark }) {
  ...
  if (!d || !sizes || sizes.length < 2 || sizes[0] <= 0) {
    return (
      ...
      >
        Run at least two input sizes to compare observed growth against the
        stated complexity.
      </p>
    );
  }
  ...
  const predicted =
    exponent == null ? null : predictedGrowth(exponent, nMin, nMax);
  ...
  return (
    ...
    >
      Observed scaling: between n={nMin.toLocaleString()} and n=
      {nMax.toLocaleString()},{" "}
      <strong style={{ color: "#1f9d48" }}>{d.name}</strong>&apos;s median{" "}
      {unit} grew approximately{" "}
      <strong style={{ color: "#c93400" }}>{observed.toFixed(1)}×</strong> —{" "}
      {verdict}. This is a single-run observation on this machine; it is not a
