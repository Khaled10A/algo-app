...
export function SortResults({
  ...
  const algoTotals = algos.map((algo) => ({
    ...
    total: types.reduce(
      (sum, t) =>
        sum + sizes.reduce((s2, _, si) => s2 + (data[algo][t][si][mk] || 0), 0),
      0,
    ),
  }));
  const worstAlgo = algoTotals.reduce(
    (a, b) => (b.total > a.total ? b : a),
    algoTotals[0],
  )?.algo;
  const bestAlgo = algoTotals.reduce(
    (a, b) => (b.total < a.total ? b : a),
    algoTotals[0],
  )?.algo;
    ...
    <div>
      ...
      >
        <div style={{ display: "flex", alignItems: "baseline", gap: 9 }}>
          <span style={{ fontSize: 11, fontWeight: 600, color: "#d70015" }}>
            Worst performer
          </span>
          <span style={{ fontSize: 15, fontWeight: 700, color: "#d70015" }}>
            {getAlgorithmForDisplay(worstAlgo).name}
          </span>
        </div>
        <div
          aria-hidden="true"
          style={{ width: 1, height: 22, background: "rgba(127,127,127,0.30)" }}
        />
        <div style={{ display: "flex", alignItems: "baseline", gap: 9 }}>
          <span style={{ fontSize: 11, fontWeight: 600, color: "#1f9d48" }}>
            Best performer
          </span>
          <span style={{ fontSize: 15, fontWeight: 700, color: "#1f9d48" }}>
            {getAlgorithmForDisplay(bestAlgo).name}
          </span>
        </div>
      </div>
      ...
      >
        <ChartBox
          ref={lineRef}
          title="Line Chart — Avg vs Size"
          onExport={() =>
            exportSVGasPNG(
              lineRef.current,
              "sort_line_chart.png",
              th === "light" ? "#ffffff" : "#0f172a",
            )
          }
          onFullscreen={() =>
            setFsChart({
        ...
        <ChartBox
          ref={barRef}
          title={`Bar Chart — ${INPUT_LABELS[types[0]]}`}
          onExport={() =>
            exportSVGasPNG(
              barRef.current,
              "sort_bar_chart.png",
      ...
      {types.map((type) => (
        <div key={type} style={{ marginBottom: 16 }}>
          ...
          >
            <thead>
              <tr style={{ background: ts.rowOdd }}>
                <th style={ts.TH}>Algorithm</th>
                {sizes.map((n) => (
                  <th key={n} style={ts.TH}>
                    n = {n}
                  </th>
                ))}
                <th style={{ ...ts.TH, color: "#8944ab" }}>
                  Basic Ops (n={sizes[sizes.length - 1]})
                </th>
                <th style={{ ...ts.TH, color: "#c93400" }}>
                  Theoretical Formula
                </th>
              </tr>
            </thead>
                  ...
                  <tr key={algo} style={{ background: rowBg }}>
                    ...
                    <td style={{ ...ts.TD, fontSize: 10 }}>
                      {(() => {
                        ...
                        if (!c) return "-";
                        const theoretical = c.worst.includes("n²")
                          ? Math.round(n * n)
                          : c.worst.includes("n log n")
...
export function SearchResults({
  ...
  if (results.mode === "file") {
    ...
    const rows = algos
      .map((algo) => {
        ...
        return {
          algo,
          name: getAlgorithmForDisplay(algo).name,
          time: r.time,
          comparisons: r.comparisons,
          matches: r.matches?.length || 0,
          color: getAlgorithmForDisplay(algo).color,
        };
      })
      .sort((a, b) => a[mk] - b[mk]);
      ...
      <div>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12, color: pf.textPrimary }}>
              {fileName}
            </div>
          </div>
          <div>
            ...
            <div style={{ fontSize: 12, color: pf.textPrimary }}>
              {fileLength.toLocaleString()} chars
            </div>
          </div>
          <div>
            <div
            ...
            <div
              style={{ fontSize: 12, color: "#b25000", fontFamily: "inherit" }}
            >
              "{pattern}"
            </div>
          </div>
          ...
          >
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr
                  key={r.algo}
                  style={{ background: i % 2 === 0 ? pf.rowEven : pf.rowOdd }}
                >
                  <td
                    style={{
                      padding: "9px 14px",
                  ...
                  <td style={{ padding: "9px 14px", color: pf.textSecondary }}>
                    {r.time.toFixed(4)} ms
                  </td>
                  <td style={{ padding: "9px 14px", color: pf.textSecondary }}>
                      ...
                      style={{
                        background:
                          r.matches > 0
                            ? "rgba(48, 209, 88, 0.14)"
                            : "rgba(255, 59, 48, 0.10)",
                        color: r.matches > 0 ? "#1f9d48" : "#d70015",
                        padding: "2px 8px",
                        borderRadius: 4,
                        fontSize: 10,
                      }}
          ...
          {rows.map((r, i) => {
            ...
            return (
              <div key={r.algo} style={{ marginBottom: 10 }}>
                ...
                >
                  <span style={{ color: r.color, fontFamily: "monospace" }}>
                    {r.name}
                  </span>
                ...
                <div
                  style={{ background: pf.trackBg, borderRadius: 4, height: 8 }}
                >
                  <div
                    style={{
                      width: `${(r[mk] / maxVal) * 100}%`,
                      height: "100%",
                      background: r.color,
                      borderRadius: 4,
  ...
  const algoTotals = algos.map((algo) => ({
    ...
    total: scenarios.reduce(
      (sum, sc) =>
        sum +
        sizes.reduce((s2, _, si) => s2 + (data[algo][sc][si][mk] || 0), 0),
      0,
    ),
  }));
  const worstAlgo = algoTotals.reduce(
  ...
  return (
    <div>
      ...
      >
        <ChartBox
          ref={lineRef}
          title="Line Chart — Avg vs Text Size"
          onExport={() =>
            exportSVGasPNG(
              lineRef.current,
              "search_line_chart.png",
              th === "light" ? "#ffffff" : "#0f172a",
            )
          }
          onFullscreen={() =>
        ...
        <ChartBox
          ref={barRef}
          title={`Bar Chart — ${SCENARIO_LABELS[scenarios[0]]}`}
          onExport={() =>
            exportSVGasPNG(
              barRef.current,
              "search_bar_chart.png",
              th === "light" ? "#ffffff" : "#0f172a",
            )
          }
          onFullscreen={() =>
            setFsChart({
              ...
              node: (
                <BarChart
                  ...
                  colors={searchColors}
                  labels={algos}
                  title={`${ml} — ${SCENARIO_LABELS[scenarios[0]]}`}
                  xTitle="Text Size"
                  yTitle={ml}
                  fullscreen
                />
