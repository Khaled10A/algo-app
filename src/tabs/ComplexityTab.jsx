...
function ComparePanel({ tab, metric }) {
  ...
  const th = useTheme();
  const pf = getPalette(th === "dark" ? "dark" : "light");
    ...
    if (tab === "sorting") {
      ...
      const res = descriptors
        .map(({ name, color, run: fn }) => {
          const { time, comparisons } = measure(fn, {
            setup: () => [arr],
            repeats: 1,
          });
          return {
  ...
  return (
    ...
    >
      <button onClick={run} style={{ ...btnStyle(th), marginBottom: 12 }}>
        Run side-by-side (n=100)
      </button>
      {result &&
        result.map((r, i) => (
          <div key={r.name} style={{ marginBottom: 9 }}>
            ...
            >
              ...
              <span style={{ color: pf.textSecondary, fontSize: 10 }}>
                {mk === "time"
                  ? r.time + " ms"
  ...
  return (
    <div>
      ...
      <div style={{ overflowX: "auto", marginBottom: 24 }}>
        ...
        >
          <thead>
            <tr style={{ background: isDark ? "#232325" : "#e8e8ed" }}>
              {[
                "Algorithm",
                "Paradigm",
                "Best",
          ...
          <tbody>
            {rows.map((d, i) => {
              ...
              return (
                <tr
                  key={d.id}
                  style={{ background: i % 2 === 0 ? ts.rowEven : ts.rowOdd }}
                >
                  <td style={{ ...ts.TD, color: d.color, fontWeight: "bold" }}>
                    {d.name}
                  </td>
                  <td
                    style={{ ...ts.TD, color: pf.textSecondary, fontSize: 11 }}
                  >
                    {c.paradigm}
                  </td>
                  ...
                  <td style={{ ...ts.TD, color: pf.textSecondary }}>
                    {c.space}
                  </td>
                </tr>
              );
