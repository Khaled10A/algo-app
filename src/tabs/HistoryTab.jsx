...
export function HistoryTab({ history, compare, setCompare, isDark }) {
  ...
  const toggleCompare = (run) => {
    setCompare((prev) => {
      ...
      if (selectedRuns.length > 0 && selectedRuns[0].kind !== run.kind)
        return prev;
  ...
  if (history.length === 0) {
    return (
      ...
      >
        ...
        <div style={{ fontSize: 11, color: pf.textSecondary }}>
          NO RUNS YET — EXECUTE A BENCHMARK
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      {compareRuns.length >= 2 && (
        ...
        >
          ...
          >
            Comparing {compareRuns.length} runs
          </div>
            ...
            >
              <thead>
                <tr>
                  ...
                  {compareRuns.map((r) => (
                    <th key={r.id} style={{ ...ts.TH, color: pf.accentText }}>
                      {r.label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {compareRuns[0].kind === "sorting" &&
                  compareRuns[0].algos.map((algo, ai) => {
                    const rowVals = compareRuns.map((r) => {
                      const mk = r.metric === "time" ? "time" : "comparisons";
                      const total = r.types.reduce(
                        (s, t) =>
                          s +
                          (r.results[algo]?.[t]?.reduce(
                            (s2, row) => s2 + (row[mk] || 0),
                            0,
                          ) || 0),
                        0,
                ...
                {compareRuns[0].kind === "search" &&
                  compareRuns[0].algos.map((algo, ai) => {
                    const searchColors = ["#f472b6", "#4ade80", "#fb923c"];
                    const rowVals = compareRuns.map((r) => {
                      const mk = r.metric === "time" ? "time" : "comparisons";
                      return r.scenarios.reduce(
                        (s, sc) =>
                          s +
                          (r.results[algo]?.[sc]?.reduce(
                            (s2, row) => s2 + (row[mk] || 0),
                            0,
                          ) || 0),
                        0,
      ...
      {compare.length > 0 && compare.length < 2 && (
        <div
          style={{
            ...
            border: `1px dashed ${pf.accent}55`,
            borderRadius: 8,
            padding: "10px 14px",
            marginBottom: 14,
            fontSize: 10,
        ...
        >
          Select {2 - compare.length} more run{compare.length === 0 ? "s" : ""}{" "}
          to compare (max 4)
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {history.map((run) => {
          ...
          const algosData =
            run.kind === "sorting"
              ? run.algos.map((a) => ({
                  id: a,
                  name: algoName(a),
                  val: run.types.reduce(
                    (s, t) =>
                      s +
                      (run.results[a]?.[t]?.reduce(
                        (s2, r) => s2 + (r[mk] || 0),
                        0,
                      ) || 0),
                    0,
              ...
              : run.algos.map((a) => ({
                  id: a,
                  name: algoName(a),
                  val: run.scenarios.reduce(
                    (s, sc) =>
                      s +
                      (run.results[a]?.[sc]?.reduce(
                        (s2, r) => s2 + (r[mk] || 0),
                        0,
                      ) || 0),
                    0,
          ...
          const worstName = algosData.reduce(
            (a, b) => (b.val > a.val ? b : a),
            algosData[0],
          )?.name;
            ...
            >
              ...
              >
                <div>
                  ...
                  <div
                    style={{ fontSize: 9, color: textMuted, letterSpacing: 1 }}
                  >
                    {run.ts} ·{" "}
                    {run.kind === "sorting" ? "SORTING" : "STRING MATCHING"} ·{" "}
                    {run.metric === "time" ? "TIME" : "COMPARISONS"}
                    {run.inputMode === "custom" && (
